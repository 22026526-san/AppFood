...
const EditFoodScreen = () => {
  ...
  const [name, setName] = useState(data_food[0].food_name)
  const [category, setCategory] = useState('');
  const [categoryId, setCategoryId] = useState(data_food[0].category_id);
  const [price, setPrice] = useState(data_food[0].price);
  const [description, setDescription] = useState(data_food[0].description);
  const [img, setImg] = useState(data_food[0].image_url);
  ...
  const getCategory = async () => {
    try {
      ...
      if (result.success) {
        ...
        updateCategoryById(result.message, data_food[0].category_id);
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/admin/update_food`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={{ fontSize: 22, color: '#000000d5', fontWeight: 'bold' }}>Edit Food</Text>
      </View>
              ...
              <View style={styles.modalContainer}>
                ...
                <View style={styles.buttonRow}>
                  <TouchableOpacity onPress={() => {
                    setCategoryId(data_food[0].category_id);
                    setModalVisible(false);
                    updateCategoryById(data_cate, data_food[0].category_id)
